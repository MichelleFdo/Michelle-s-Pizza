			...
			if (isset($document->helper) && $document->helper instanceOf JSNTplTemplateHelper && $document->compression > 0)
			{
				...
				if ($config->get('ftp_enable') OR is_writable($cachePath))
				{
					...
					if ($document->compression == 1 OR $document->compression == 2)
					{
						...
						{
							if (version_compare($JVersion->getShortVersion(), '3.7', '>='))
							{
								$tmpCompressedStyleSheet = array(
										'type' => 'text/css',
										'options' => array('version' => 'auto')
								);

								if ($compressedStyleSheet['media'] != '')
								{
									$tmpCompressedStyleSheet ['media'] = $compressedStyleSheet['media'];
								}

								$stylesheets[$compressedStyleSheet['file']] = $tmpCompressedStyleSheet;
							}
							else
							{
	...
	public static function onAfterRender ()
	{
		...
		if ($app->isAdmin())
		{
			...
			if (isset(self::$_templateAdmin) AND self::$_templateAdmin instanceOf JSNTplTemplateAdmin)
			{
				...
				// Clean-up Chosen calls if running on Joomla 3.7
				if (version_compare($JVersion->getShortVersion(), '3.7', '>='))
				{
					$html = call_user_func($get);

					if (preg_match('#initChosen\(\);#', $html, $match))
					{
						$html = str_replace($match[0], '', $html);
					}

					if (preg_match('#\$\("body"\).on\("subform-row-add", initChosen\);#', $html, $match))
					{
						$html = str_replace($match[0], '', $html);
					}

					if (preg_match('#<script[^>]+src="[^"]*/media/jui/js/chosen\.jquery(\.min)?\.js(\?(.*))?"[^>]*></script>#', $html, $match))
					{
						$html = str_replace($match[0], '', $html);
					}

					if (preg_match('#[\r\n][\s\t]+<link href="[^"]*/media/jui/css/chosen\.css(\?(.*))?" rel="stylesheet" />#', $html, $match))
					{
						$html = str_replace($match[0], '', $html);
					}
		...
		elseif ($templateInfo = JSNTplTemplateRecognization::detect())
		{
			//replace Favicon
			if ($app->isSite())
			{
			...
			if (strtolower($templateInfo->edition) == 'free' && $app->isSite())
			{
				preg_match('#<div id="jsn-brand">(.*)</div>#si', $html, $out);

				if (count($out))
				{
					$copyright = 'JSN ' . $templateInfo->name .' is designed by <a href="https://www.joomlashine.com" title="JoomlaShine is the best Joomla templates & extension provider" rel="nofollow" target="_blank">JoomlaShine.com</a>';
					if (trim($out[1]) != '')
					{
						$html = str_replace($out[1], $copyright, $html);
					}
					elseif(trim($out[0]) != '')
					{
						$html = str_replace($out[0], $copyright, $html);
					}
					else
					{
						//do nothing
