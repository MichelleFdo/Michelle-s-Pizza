...
class JSNTplUtils
{
	...
	/**
	 * Check if Advanced Templates Manager is installed or not.
	 *
	 * @return  boolean
	 */
	public function checkAdvancedTemplatesManager()
	{
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->clear();
		$query->select('COUNT(*)');
		$query->from('#__extensions');
		$query->where('type = ' . $db->quote('component') . ' AND element = ' . $db->quote('com_advancedtemplates'));
		$db->setQuery($query);
		return (int) $db->loadResult();
	}
}
